...
2. Когда клиент пишет вам — бот засекает таймер (настраивается, по умолчанию 10 мин)
...
Бот повторяет напоминания несколько раз (настраивается, по умолчанию макс. 6), затем перестаёт беспокоить.

### 🕐 Тихие часы

Можно настроить **рабочие часы** и **дни недели**, когда должны приходить напоминания. В нерабочее время:

- Сообщения клиентов не теряются — они накапливаются в дайджесте
- При начале активного периода бот присылает **сводку пропущенных сообщений**
- Напоминания возобновляются автоматически
...
- `/settings` — настройки (интерактивное меню)
...
| Параметр | Описание |
|---|---|
| 🕐 **Начало / Конец** | Рабочие часы (выбор с шагом 30 мин) |
| 📅 **Дни недели** | ПН–ВС, чекбоксы |
| 🔁 **Макс. повторов** | Сколько раз напоминать (1–30) |
| 🔔 **Задержка** | Через сколько минут после сообщения напомнить (5–120) |
...
# REMINDER_MINUTES=10             # задержка напоминания по умолчанию (мин)
...
├── secrets/                   # ToDo: добавить обработку входящих
